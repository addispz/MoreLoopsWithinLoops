...
def run_test_largest_negative_number():
    ...
    # Test 3:
    expected = -2
    answer = largest_negative_number(([1, 2, -23], [-2, 4, 6], [34, -32, 45]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for j in range(len(seq_seq[k])):
            if largest_negative is None and seq_seq[k][j] < 0:
                largest_negative = seq_seq[k][j]
            if largest_negative is not None and seq_seq[k][j] > largest_negative and seq_seq[k][j] < 0:
                largest_negative = seq_seq[k][j]
